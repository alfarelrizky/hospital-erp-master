...
class perawatan extends CI_Controller
{
	...
	public function act_tambah()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$cekin = $this->Model_rawat->insert_perawatan($id, $namadokter, $tanggalperiksa,$tindakan,$obat,$anamase,$diagnosis,$statuspasien);
			if ($cekin == true) {
	...
	public function edit($id){
		$data['data'] = $this->Model_rawat->select_where_perawatan($id)->result_array();
	...
	public function act_edit(){
		...
		//cek
		if ($this->form_validation->run() == FALSE) {
			$data['data'] = $this->Model_rawat->select_where_perawatan($this->input->post('id'))->result_array();
			$this->load->view('view_form_edit_perawatan', $data);
		} else {
			...
			$cek = $this->Model_rawat->update_perawatan($i,$id, $namadokter, $tanggalperiksa, $tindakan, $obat, $anamase, $diagnosis, $statuspasien);;
			if ($cek == true) {
				$this->session->set_flashdata('alert-stat', 'BERHASIL');
	...
	{
		$cek = $this->Model_rawat->delete_perawatan($id);
		if ($cek == true) {
			$this->session->set_flashdata('alert-stat', 'BERHASIL');
